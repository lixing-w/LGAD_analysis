...
# Dev logs
    ...
    Aug 6 (Lixing)
        - code adjustment and simplification for analyze_sensor_iv in analyze.py
        - added option to choose env var to analyze, in analyze_sensor_iv and 
          command line argument
    
    Aug 11 (Lixing)
        - wrote predict.py that feeds the models with env vars and make them predict
          the IV curve
        - Note: the pipeline is env vars -> MLP -> latent vec -> AE -> whole curve
        - AE now outputs two channels, one for current, one for alpha, so we know 
          when the prediction ends
        - AE now takes in two channels, one for current, one for voltage, to help 
          model determine the correct voltage for breakdown (it was confused by 
          similar curves but with different breakdowns, you can see the prediction
          moves upward a bit then come down before the "final" breakdown)
        - the regressor in AE now also predicts breakdown voltage (extracted by RANSAC)
          to help reconstruction.
        - the model is suffering severely from overfitting and unbalanced data 
          distribution. To remedy, I tried VAE (variational autoencoder) to improve 
          generalization. But VAE are much worse at reconstruction than AE. It 
          tries to obtain a smoother latent space to reduce overfitting and improve
          generalization, but maybe the problematic data distribution made such 
          "smooth" latent space not possible.

    Aug 12 (Lixing)
        - tried larger latent size for better reconstruction
        - added and experimented with self-attention layers
        - the regressor in AE now also predicts the slope and offset of the 
          primary fit curve extracted by RANSAC.
