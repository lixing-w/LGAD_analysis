...
def train_end_to_end(autoencoder_model_path: str, reconstruction_weight: float = 1.0, latent_weight: float = 1.0, is_conditional: bool = False):
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_seq, humi, ramp_type, dura, seq_len, bd_v, sensor_num, sensor_name, sensor_thickness, sensor_type, gt_latent in tqdm(train_loader):
            # Prepare inputs - handle inf values
            if sensor_thickness == float('inf'): sensor_thickness = torch.tensor([0.0])
            ...
            metrics = torch.stack([temp, humi, ramp_type, sensor_num, sensor_thickness, sensor_type], dim=1).float().to(device)
            optimizer.zero_grad()
            ...
            l1_penalty = sum(p.abs().sum() for p in mlp_model.parameters())
            total_loss = reconstruction_weight * reconstruction_loss + latent_weight * latent_loss + config['l1_lambda'] * l1_penalty
...
def train(autoencoder_model_path: str):
    ...
    print(f"Using device: {device}")
    dataset = AggregateLatentDataset(DATABASE_DIR, autoencoder_model_path, is_conditional=True, data_config_path="./ml_data_config.txt")
    train_loader = DataLoader(dataset, batch_size=1, shuffle=True)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_curve, humi, ramp_type, dura, seq_len, bd_v, sensor_num, sensor_name, t_latent in tqdm(train_loader):
            ...
            loss = criterion(p_latent, t_latent) + config['l1_lambda'] * l1_penalty
            loss.backward()
...
def explain(model_path: str, autoencoder_model_path: str):
    ...
    selected_metrics = set([0,2,3,4,8,10,11]) # temp, humi, ramp_type, sensor_num, sensor_thickness, sensor_type
    ...
    input_names = ["Temperature", "Humidity", "Ramp Type", "Sensor Number", "Sensor Thickness", "Sensor Type"]
...
if __name__ == "__main__":
    ...
    # train_end_to_end("conditional_autoencoder_model/ivcvscans-2025-10-13-14-25-08/e97_l0.269.pth", reconstruction_weight=2.0, latent_weight=1.0, is_conditional=True)  # latent 16 + params 6
    # train_end_to_end("conditional_autoencoder_model/ivcvscans-2025-10-28-10-30-59/e95_l0.211.pth", reconstruction_weight=2.0, latent_weight=1.0, is_conditional=True)  # latent 8 + params 6
    # train_end_to_end("conditional_autoencoder_model/ivcvscans-2025-10-28-11-02-38/e97_l0.305.pth", reconstruction_weight=2.0, latent_weight=1.0, is_conditional=True)  # latent 4 + params 6
    
    train_end_to_end("conditional_autoencoder_model/ivcvscans-2026-01-07-21-41-28/e999_l0.177.pth", reconstruction_weight=2.0, latent_weight=1.0, is_conditional=True)