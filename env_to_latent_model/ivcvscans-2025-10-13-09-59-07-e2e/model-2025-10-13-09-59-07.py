...
class EnvToLatent(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.latent_dim = 16  # must match with AutoEncoder
...
class ConditionalAutoEncoder(nn.Module):
    ...
    def __init__(self, max_seq_len):
        ...
        self.latent_dim = 16  # Reduced from 20
        self.param_dim = 6    # [temp, date, humi, ramp_type, dura, sensor_num]
        self.decoder_input_dim = self.latent_dim + self.param_dim  # 22
        ...
        self.encoder_fc = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.LayerNorm(128),
            nn.Linear(128, self.latent_dim)  # Reduced to 16
        )
        
        # Decoder takes concatenated latent + parameters (16 + 6 = 22)
        self.decoder = nn.Sequential(
            nn.Linear(self.decoder_input_dim, 128),  # 22 -> 128
            nn.LeakyReLU(),
            nn.LayerNorm(128),
            nn.Linear(128, self.flattened_dim),
            nn.Unflatten(1, (self.cnn_out.shape[1], self.cnn_out.shape[2])),
