...
def criterion(output, target, seq_len, mu, logvar, max_seq_len, device):
    ...
    return curve_loss + kl_loss * 0.2 + mask_loss * 0.5 + nonmono_loss * 0.3
...
def aggregate_train():
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_curve, humi, ramp_type, dura, seq_len, sensor_num, sensor_name in tqdm(train_loader):
            ...
            loss = criterion(recons, i_curve, seq_len, mu, logvar, dataset.max_seq_len, device)
            loss.backward()
...
if __name__ == '__main__':
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-11-22-47-16/e88_l3.679.pth")