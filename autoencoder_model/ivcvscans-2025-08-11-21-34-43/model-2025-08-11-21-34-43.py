...
class VAE(nn.Module):
    def __init__(self, max_seq_len):
        ...
        self.encoder_cnn = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.LayerNorm([128, max_seq_len // 1]),
            nn.Conv1d(128, 128, 3, stride=2, padding=1),
            nn.LeakyReLU(),
            nn.LayerNorm([128, max_seq_len // 2]),
            nn.Conv1d(128, 256, 3, stride=2, padding=1),
        ...
        self.mu_fc = nn.Sequential(
            nn.Linear(128, 128),
            nn.Linear(128, self.latent_dim), # predicts mu of p(z|x)
        )
        self.logvar_fc = nn.Sequential(
            nn.Linear(128, 128),
            nn.Linear(128, self.latent_dim) # predicts log(std^2) of p(z|x)
        )
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.LayerNorm([128, max_seq_len // 1]),
            nn.ConvTranspose1d(128, 32, 3, stride=1, padding=1, output_padding=0),
            nn.LeakyReLU(),
            nn.ConvTranspose1d(32, 2, 3, stride=1, padding=1, output_padding=0), # 2 output channels, 1 for current, 1 for end_prob
        )
