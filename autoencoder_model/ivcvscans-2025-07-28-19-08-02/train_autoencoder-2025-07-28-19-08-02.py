...
def criterion(output, target, seq_len):
    ...
    mse_2deriv = torch.mean(torch.abs(torch.diff(output, n=2)[:,:,40:int(seq_len*0.8)])) # [] #[:,:,30:120]
    
    bias_loss = torch.mean(torch.abs(torch.mean(output - target, dim=2)))
    return mse + mse_1deriv + mse_2deriv * 0.6 + bias_loss #+ smooth_diff * 0.1 + smooth_coeff * 0.025 # 0.025
...
aggregate_train()
# aggregate_run("autoencoder_model/ivcvscans-2025-07-28-18:16:03/e296_l0.051.pth")