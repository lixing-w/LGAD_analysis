...
sigmoid = nn.Sigmoid()
relu = nn.ReLU()
def criterion(epoch, output, target, seq_len, mu, logvar, max_seq_len, device):
    """
    Loss function
    """
    ...
    # 2. then find the kl_loss between q(z|x) and p(z)
    kl_loss = -0.5 * torch.sum(1 + logvar - torch.square(mu) - torch.exp(logvar))
    kl_coeff = epoch / 80 if epoch <= 80 else 1
    ...
    # 4. punish non-monotonic prediction (after depletion)
    # nonmono_loss = torch.mean(torch.square(relu(-current_diff[:,:,30] - 1e-3)))
    return curve_loss + kl_loss * (kl_coeff * 0.1) + mask_loss * 0.5, kl_loss
...
def aggregate_train():
    ...
    # initialize model 
    model = VAE(dataset.max_seq_len).to(device)
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_curve, humi, ramp_type, dura, seq_len, sensor_num, sensor_name in tqdm(train_loader):
            ...
            recons, mu, logvar = model(i_curve)
            
            loss, kl_loss = criterion(epoch, recons, i_curve, seq_len, mu, logvar, dataset.max_seq_len, device)
            loss.backward()
        ...
        print(f"Epoch {epoch}, Loss: {avg_loss:.4f}, KL_loss: {avg_kl_loss:.4f}, lr: {optimizer.param_groups[0]['lr']}")
...
def aggregate_run(model_path: str):
    ...
    with torch.no_grad():
        for i, idx in enumerate(idxs):
            ...
            volts = orig[:,0]
            plt.plot(volts, orig[:,1], color="black", label="Original")
            curr = i_recons[:,0].squeeze().cpu().detach().numpy()[:seq_len].transpose()
...
if __name__ == '__main__':
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-12-00-56-16/e53_l2.667.pth")