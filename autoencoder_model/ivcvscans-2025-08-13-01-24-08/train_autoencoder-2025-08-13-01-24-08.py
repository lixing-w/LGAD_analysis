...
def criterion(output, target, seq_len, pred_metrics, target_metrics, max_seq_len, device):
    ...
    return curve_loss + mask_loss * 0.5 + metrics_loss * 0.2 + line_param_loss * 0.3
...
def aggregate_train():
    ...
    train_loader = DataLoader(dataset, batch_size=config['batch_size'], shuffle=True, num_workers=config['batch_size']*2, persistent_workers=True)
...
if __name__ == '__main__':
    ...
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-13-01-14-36/e88_l1.074.pth")
