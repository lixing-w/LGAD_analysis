...
def criterion(output, target, seq_len, pred_metrics, target_metrics, max_seq_len, device):
    ...
    # broadcast
    true_mask = (positions < seq_len.unsqueeze(1)).float()
    ...
    mse2weight = ((positions < (seq_len.unsqueeze(1) * 0.9)) & (positions > 40)).float()
    mse_2deriv = torch.mean(torch.abs(torch.diff(current, n=2) * mse2weight[:, :-2]))  # [] #[:,:,30:120]
    ...
    curve_loss = mse + mse_1deriv + mse_2deriv + l1_loss * 1.7 + bias_loss * 4 + mono_loss  # + smooth_diff * 0.1 + smooth_coeff * 0.025 # 0.025

    # 3. then find loss of metrics prediction
    mask = ~(torch.isnan(target_metrics) | torch.isinf(target_metrics))
    loss = torch.mean(torch.square((pred_metrics - target_metrics) * mask))
    line_param_loss = torch.mean(torch.square(pred_metrics[:, [5, 6, 7, 8]] - target_metrics[:, [5, 6, 7, 8]]))
    metrics_loss = loss + line_param_loss * 2
...
def aggregate_train():
    ...
    config = {
        ...
        'batch_size': 2,  # Single video per batch
...
if __name__ == '__main__':
    ...
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-12-23-53-59/e116_l18.583.pth")
