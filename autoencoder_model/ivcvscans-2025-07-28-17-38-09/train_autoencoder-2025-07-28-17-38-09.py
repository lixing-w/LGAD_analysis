...
def train(sensor_name: str):
    ...
    def criterion(output, target, seq_len):
        ...
        mse_2deriv = torch.mean(torch.abs(torch.diff(output, n=2))) #[:,:,30:120]
        
        bias_loss = torch.mean(torch.abs(torch.mean(output - target, dim=2)))
        return mse + mse_1deriv + mse_2deriv * 0.5 + bias_loss #+ smooth_diff * 0.1 + smooth_coeff * 0.025 # 0.025
...
aggregate_train()
# aggregate_run("autoencoder_model/ivcvscans-2025-07-28-16:29:42/e170_l0.087.pth")