...
sigmoid = nn.Sigmoid()
relu = nn.ReLU()
def criterion(output, target, seq_len, mu, logvar, max_seq_len, device):
    """
    Loss function
    """
    ...
    return curve_loss + kl_loss * 0.1 + mask_loss * 0.5, kl_loss
...
def aggregate_train():
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_curve, humi, ramp_type, dura, seq_len, sensor_num, sensor_name in tqdm(train_loader):
            ...
            loss, kl_loss = criterion(recons, i_curve, seq_len, mu, logvar, dataset.max_seq_len, device)
            loss.backward()
...
if __name__ == '__main__':
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-12-00-40-11/e85_l2.395.pth")