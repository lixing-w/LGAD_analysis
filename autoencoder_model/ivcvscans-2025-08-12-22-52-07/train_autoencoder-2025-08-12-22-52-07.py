...
def criterion(output, target, seq_len, pred_metrics, target_metrics, max_seq_len, device):
    ...
    curve_loss = mse + mse_1deriv + mse_2deriv + l1_loss * 1.7 + bias_loss * 4  # + smooth_diff * 0.1 + smooth_coeff * 0.025 # 0.025
    ...
    date_sensor_num_loss = torch.mean(ptwise_se(pred_metrics[:, [1, 9]], target_metrics[:, [1, 9]]))
    humi_loss = 0 if target_metrics[:, 2] == float('inf') else torch.square(pred_metrics[:, 2] - target_metrics[:, 2])
    ramp_type_loss = 0 if target_metrics[:, 3] == float('inf') else torch.square(
        pred_metrics[:, 3] - target_metrics[:, 3])
    ...
    metrics_loss = temp_loss + date_sensor_num_loss + humi_loss + ramp_type_loss + dura_loss + line_param_loss
...
def aggregate_train():
    ...
    for epoch in range(config['num_epochs']):
        ...
        for temp, date, iv_curve, humi, ramp_type, dura, seq_len, bd_v, i_at_100v, slope, offset, sensor_num, sensor_name in tqdm(
            ...
            loss = criterion(recons, iv_curve[:,[1],:], seq_len, p_metrics, t_metrics, dataset.max_seq_len, device)
            loss.backward()
...
if __name__ == '__main__':
    ...
    aggregate_train()
    # aggregate_run("autoencoder_model/ivcvscans-2025-08-12-22-11-04/e141_l32.848.pth")
