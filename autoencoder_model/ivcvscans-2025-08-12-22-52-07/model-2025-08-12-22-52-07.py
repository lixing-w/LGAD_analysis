...
class SelfAttention1D(nn.Module):
    def __init__(self, in_channels, d_k):
        super().__init__()
        self.query_conv = nn.Conv1d(in_channels, d_k, kernel_size=1)
        self.key_conv = nn.Conv1d(in_channels, d_k, kernel_size=1)
        self.value_conv = nn.Conv1d(in_channels, d_k, kernel_size=1)
        self.out_conv = nn.Conv1d(d_k, in_channels, kernel_size=1)
        self.scale = np.sqrt(d_k)

    def forward(self, x):   # x shape: (B, C, L)
        # Q, K, V each has shape (B, d_k, L)
        Q = self.query_conv(x)
        K = self.key_conv(x)
        V = self.value_conv(x)

        # Compute attention scores
        # transpose Q for matrix multiplication: (B, L, d_k)
        scores = torch.bmm(Q.transpose(1, 2), K) / self.scale  # (B, L, L)
        attn = F.softmax(scores, dim=-1)  # attention weights

        # Weighted sum of values
        out = torch.bmm(V, attn.transpose(1, 2))  # (B, d_k, L)
        # project back to input channels
        out = self.out_conv(out)  # (B, C, L)
...
class AutoEncoder(nn.Module):
    ...
    def __init__(self, max_seq_len):
        ...
        self.encoder_cnn = nn.Sequential(
            ...
            SelfAttention1D(128, 32),
            nn.Conv1d(128, 128, 3, stride=2, padding=1),
            nn.LeakyReLU(),
            nn.LayerNorm([128, max_seq_len // 4]),
            nn.Conv1d(128, 256, 3, stride=2, padding=1),
            nn.LeakyReLU(),
            SelfAttention1D(256, 32),
        )

        dummy_input = torch.zeros(1, 2, max_seq_len)  # batch_size=1
        with torch.no_grad():
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(),
            SelfAttention1D(32, 16),
            nn.ConvTranspose1d(32, 2, 3, stride=1, padding=1, output_padding=0),
            # 2 output channels, 1 for current, 1 for end_prob
        )
        ...
        # [temp, date.toordinal(), humi, ramp_type, duration, bd_v, i_at_100v, slope, offset, sensor_number]
...
class VAE(nn.Module):
    def __init__(self, max_seq_len):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.ConvTranspose1d(64, 2, 3, stride=1, padding=1, output_padding=0),
            # 2 output channels, 1 for current, 1 for end_prob
        )
...
class EnvToLatent(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.latent_dim = 30  # must match with AutoEncoder
